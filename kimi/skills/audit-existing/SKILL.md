---
name: audit-existing
description: Audit a repository before planning or extending it. Produces an implemented/missing/duplicated/broken
  checklist without editing files.
whenToUse: Audit a repository before planning or extending it. Produces an implemented/missing/duplicated/broken
  checklist without editing files.
---
...
- Before `/skill:execute-prd` drafts a plan from a PRD/prompt/RFC.
...
- The repo is provably greenfield (an empty workspace stub) and the
  requirement is to scaffold from zero — `/skill:execute-prd` will say so and
  skip the audit.
...
- Do not duplicate work `/skill:domain-review` does. Audit is *what exists*, not
...
- **Postconditions:** caller (`/skill:execute-prd`, `/skill:modernize`) consumes the audit as a plan input; the audit reports state-of-the-repo, not opinions about state-of-the-repo.
- **Failure modes:** repo unreadable → halt; asked to propose fixes → refuse and surface the request as a finding (fixes belong to the planner); duplication with `/skill:domain-review` requested → refuse — audit is "what exists", review is "what's wrong with what exists".
