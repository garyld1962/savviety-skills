---
name: validate-plan
description: 'Lightweight readiness gate for a plan file: checks structure, task discreteness,
  acceptance criteria, milestones, and forbidden placeholders.'
whenToUse: 'Lightweight readiness gate for a plan file: checks structure, task discreteness,
  acceptance criteria, milestones, and forbidden placeholders.'
arguments:
- planPath
---
...
# /skill:validate-plan — Plan Readiness Gate
...
- Before invoking `/skill:execute-plan` (called automatically by execute-plan's Phase 0).
...
verifies plans authored by `/skill:execute-prd` pass this check without
modification.
...
Plans authored by `/skill:execute-prd` include a `## Parallel Execution`
...
`/skill:execute-plan` can rely on it instead of re-planning concurrency.
...
Fix the above and re-run /skill:validate-plan.
```
...
A `VERDICT: FAIL` from this skill causes `/skill:execute-plan` preflight
...
- **Postconditions:** caller (`/skill:execute-plan` preflight gate 2, `/skill:execute-prd` step 7) decides whether to proceed (`PASS`), warn (`WARN`), or fix (`FAIL`).
