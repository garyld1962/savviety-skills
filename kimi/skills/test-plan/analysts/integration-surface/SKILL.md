...
- Ontology slice (optional): the `## Fact Types` rows that cross a system edge —
  ...
  copied verbatim with their `#`, plus the `## Entity Types` rows (reference
  schemes) of every entity those fact types name
...
fact type names, and each entity the fact names is referenced by the reference
scheme its `## Entity Types` row declares — those rows travel in the slice
alongside the fact-type rows. The spec's rationale (`traceability`) cites the
...
- **Inputs:** router files, service files, Zod schemas bound to procedures, auth middleware definitions, CLAUDE.md conventions; ontology slice (optional): `## Fact Types` rows crossing a system edge (an entity on one side is external, or the fact is exchanged over an API), plus the `## Entity Types` rows (reference schemes) of every entity those fact types name.
