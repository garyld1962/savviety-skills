...
description: Interrogate to discover the single central thesis of a planned YouTube
  video, then produce an outline where every segment serves that thesis.
whenToUse: Interrogate to discover the single central thesis of a planned YouTube
  video, then produce an outline where every segment serves that thesis.
---


# /skill:what-is-it-about — YouTube Video Thesis & Outline
...
- User says they're planning a new YouTube video
...
The skill has three phases, run in strict order. Do not skip ahead.
...
If any test fails, keep interrogating. Don't advance to Phase 2.
...
Be willing to say *"I think this one goes."* The skill earns trust by pushing back.
...
- **Propose, don't just ask.** When you can see the thesis forming, draft it and let the user react. Faster than pure Socratic.
...
- **Stop when done.** When the outline is produced and the user confirms, stop. Don't keep grilling.
