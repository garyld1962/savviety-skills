---
name: gh-readiness
description: Verify GitHub CLI (gh) is installed, authenticated, and able to reach
  github.com before running skills that create PRs, issues, or releases. Use before
  /pr, /ship, /hotfix, /issue-slices, /bug-session, or /changelog when gh status is
  unknown.
whenToUse: Verify GitHub CLI (gh) is installed, authenticated, and able to reach github.com
  before running skills that create PRs, issues, or releases. Use before /pr, /ship,
  /hotfix, /issue-slices, /bug-session, or /changelog when gh status is unknown.
---


# /skill:gh-readiness — GitHub CLI Readiness Gate
...
- Before `/skill:pr`, `/skill:ship`, `/skill:hotfix`, `/skill:issue-slices`, `/skill:bug-session`, or `/skill:changelog` when you are unsure whether `gh` is set up.
...
Next steps:
  <PASS>    gh is ready; proceed with /skill:pr, /skill:ship, /skill:issue-slices, etc.
...
Other skills should call `/skill:gh-readiness` as a pre-flight gate when `gh` is required:

- `/skill:pr`, `/skill:ship`, `/skill:hotfix` — require `repo` scope.
- `/skill:issue-slices`, `/skill:bug-session` — require `repo` scope.
- `/skill:changelog` when releasing — requires `repo` scope.
