---
name: dep-migrate
description: 'Plan a dependency migration: analyze breaking changes, assess impact
  on your codebase, produce a step-by-step migration plan. Use when upgrading Node,
  TypeScript, frameworks, or major dependencies.'
whenToUse: 'Plan a dependency migration: analyze breaking changes, assess impact on
  your codebase, produce a step-by-step migration plan. Use when upgrading Node, TypeScript,
  frameworks, or major dependencies.'
---


# /skill:dep-migrate — Dependency Migration Planner
...
- When `/skill:dep-audit` flags a major version update
...
```
/skill:dep-migrate typescript 5.3 5.7
/skill:dep-migrate node 20 22
/skill:dep-migrate react 18 19
/skill:dep-migrate vitest 1 2
/skill:dep-migrate eslint 8 9 --scope=packages/api
```
...
- `--batch` — for large migrations (21+ files affected), execute the plan via `/batch` instead of `/skill:execute-plan`, running each migration phase in an isolated worktree with per-phase PRs
...
Migration Analysis: <package> <from> → <to>
  ...
  Next: review the plan, then:
    - Small/Medium (≤20 files): /skill:execute-plan <plan-path>
