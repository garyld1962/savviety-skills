---
name: hotfix
description: 'Apply an expedited fix for a critical production issue: branch from
  main, apply a scoped fix, run targeted tests, and fast-track merge.'
whenToUse: 'Apply an expedited fix for a critical production issue: branch from main,
  apply a scoped fix, run targeted tests, and fast-track merge.'
type: flow
disableModelInvocation: false
---


# /skill:hotfix -- Expedited Production Fix
...
- The bug is not critical -- use /skill:triage then the normal pipeline
- You don't understand the root cause yet -- use /skill:triage first
- The fix requires significant refactoring -- use the normal pipeline

## Usage

```
/skill:hotfix "describe what is broken and how to fix it"
/skill:hotfix "login 500s on expired refresh token" --from v2.4.1
/skill:hotfix "payment rounding error" --ado 12345
/skill:hotfix "auth bypass on API endpoint" --linear BF-99
```
...
---
*Created via /skill:hotfix -- expedited production fix*
EOF
)"
```
...
1. **Speed over ceremony.** Skip full /skill:domain-review and multi-specialist analysis. Keep the security check.
...
- **Inputs:** bug description; optional `--ado <id>` / `--linear <id>` (delegates to `/skill:work-item`). Calls `_internal/security-quick-check` (mandatory, all 7 points).
