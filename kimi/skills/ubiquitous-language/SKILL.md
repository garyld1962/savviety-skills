...
description: Use when you need a domain glossary. Derives UBIQUITOUS_LANGUAGE.md from
  the PRD's ONTOLOGY.md; falls back to extracting terms from conversation when no
  ontology exists.
whenToUse: Use when you need a domain glossary. Derives UBIQUITOUS_LANGUAGE.md from
  the PRD's ONTOLOGY.md; falls back to extracting terms from conversation when no
  ontology exists.
---
...
`/skill:prd-create` Step 8 once the ontology is written.

## When to Use

- After `/skill:prd-create` or `/skill:prd-validate` has produced a `docs/prds/<slug>/ONTOLOGY.md` and you want the readable glossary
...
- You want to *build* or *extend* the ontology — that is an interview, use `/skill:prd-create`
- You want the ontology rubric itself (item states, format, scoring) — see `claude/_internal/ontology-readiness/SKILL.md`
- You want the closure pass over the PRD's nouns and verbs — use `/skill:prd-validate`

## Arguments

- `--from-ontology [path]` — **default mode.** Derive the glossary from an `ONTOLOGY.md`. With no `[path]`, use the sibling `ONTOLOGY.md` of the resolved requirements artifact — `docs/prds/<slug>/ONTOLOGY.md`. Artifact resolution order is defined once, in `/skill:prd-validate` § *Arguments*; follow it there rather than restating it.
...
2. The rubric's homonym/synonym findings — `_internal/ontology-readiness` § *Elicitation Categories* (Homonyms/synonyms) and its high-risk ambiguity categories, as reported by `/skill:prd-validate` or the rubric's automated check.
...
Close the summary by suggesting `/skill:prd-create` if the domain warrants an
ontology — the glossary derived from one is the artifact that survives.

## Rules

- **The ontology wins.** When `ONTOLOGY.md` exists, it is the source of truth: derive, do not re-elicit, and do not contradict it. If a term in conversation disagrees with the ontology, that is a finding for `/skill:prd-create --extend`, not an edit here.
...
> "`ONTOLOGY.md` exists at `docs/prds/<slug>/ONTOLOGY.md`. The glossary is derived from it and is never hand-edited — re-run with `--from-ontology`. To change a definition, change the ontology (`/skill:prd-create --extend`)."
...
- **Inputs:** in derived mode, an `ONTOLOGY.md` — the `--from-ontology [path]` argument, or the sibling of the requirements artifact resolved per `/skill:prd-validate` § *Arguments*. In legacy mode, the conversation, plus the codebase under `--scan` and an existing `UBIQUITOUS_LANGUAGE.md` under `--update`.
