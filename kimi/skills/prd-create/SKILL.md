---
name: prd-create
description: 'Create a PRD, ONTOLOGY.md and AERS from an idea, goal, thesis or codebase
  via interview. Modes: greenfield, feature, refresh, rewrite. Not for an existing
  artifact (use /prd-validate).'
whenToUse: 'Create a PRD, ONTOLOGY.md and AERS from an idea, goal, thesis or codebase
  via interview. Modes: greenfield, feature, refresh, rewrite. Not for an existing
  artifact (use /prd-validate).'
---


# /skill:prd-create — PRD, Ontology and AERS Authoring Interview
...
interactive front of the requirements pipeline; `/skill:prd-validate` gates what it
produces.
...
- An idea, a `/skill:goal` statement, or a `/skill:thesis` exists but no requirements artifact does
...
- A requirements artifact already exists → `/skill:prd-validate` (it hardens; this skill drafts)
- Only the goal is unclear → `/skill:goal` first
- Scope is wobbly and there is no thesis → `/skill:thesis` first (optional)
- Structural refactor with no user-visible behaviour change → `/skill:modernize`
- You want tickets → `/skill:issue-slices`, after this
- You want to build → `/skill:execute-prd`, after this
...
- `--from <file>` — seed from a `/skill:goal` statement, a `/skill:thesis` artifact, or free notes; classified by content (see **Input Modes**).
...
- `--full-spec` — accepted for one release as the blank-start batched alias absorbed from `/skill:prd-validate`. Batching applies to Step 4 only.
...
### From `/skill:goal` or `/skill:thesis` output (`--from`)
...
1. **Interactive check.** This is an interview. In a non-interactive context (CI,
   ...
   `/skill:prd-validate`'s non-interactive failure mode: say so, and point at
...
2. **Resolve mode** — first match wins; then confirm with one question whose
   ...
   user-visible behaviour change, say so and offer `/skill:modernize` — link it, do
   not invoke it.
3. **Locate the ontology for `--extend`.** Read its header. If it has no `uod:`
   ...
   the delta cannot be classified. Say that, and point at `/skill:prd-validate`.
...
5. **Extended-thinking gate** (as `/skill:goal` Step 2). Before asking anything, reason
...
- `/skill:audit-existing` → `## Existing State` feeds AERS **Repo Starting State**,
...
- `refresh` only: `_internal/modernization-rubric` §1 shape detection (language,
  ...
  context guardrail. Do not run `/skill:modernize`: it produces a refactor plan, not a PRD.
...
offer `/skill:thesis` — link it, do not run it inline.
...
- **Explore before asking** (`/skill:grill-me`): if the Step 1 seed already answers a
...
- Say the cost of deferral out loud: only `settled` rows are consumed downstream
  by `/skill:prd-acceptance` and `/skill:test-plan`.
...
Second extended-thinking gate — the `/skill:prd-validate` Step 2.5 questions: which
...
- Acceptance criteria are `- [ ]` checkboxes carrying the load-bearing ontology
  ...
  transition criterion) so `/skill:prd-acceptance` and `/skill:test-plan` can find them.
...
Recommend `/skill:prd-validate` only when the verdict is not `Ready`, or when
...
Invoke `/skill:ubiquitous-language --from-ontology <out>/ONTOLOGY.md`, which writes
...
- ...
Next step:
  /skill:prd-validate docs/prds/<slug>/AERS.md   (if not Ready, or unknown rows remain)
  /skill:execute-prd docs/prds/<slug>/AERS.md    (if Ready)
  /skill:issue-slices                            (tickets first)
```
...
| PRD-specific sections | — | Current State → Target State | Current State → Target State with shape header; redirect to `/skill:modernize` when no behaviour changes | Current State → Target State, plus `What May Change` and the preserved list, both recording the Step 2 agreement |
...
- Do NOT run from a **non-interactive** context, and do NOT let another skill
  auto-invoke this one. `/skill:kickoff` and `/skill:execute-prd` never auto-invoke it.
...
- Do NOT drift into `/skill:execute-prd` planning or `/skill:modernize` refactor work; this
