---
name: kickoff
description: 'Use for standard feature work to autonomously ship a PRD/story/AERS:
  readiness → plan → implement → review. Does not produce audit-grade governed artifacts.'
whenToUse: 'Use for standard feature work to autonomously ship a PRD/story/AERS: readiness
  → plan → implement → review. Does not produce audit-grade governed artifacts.'
type: flow
disableModelInvocation: false
---


# /skill:kickoff — Autonomous Development Start
...
- **Inputs:** path to a requirements artifact (or `AERS.md` by default if it exists); optional `--skip-readiness`. Calls `_internal/aers-readiness` (readiness scoring), `/skill:prd-validate` (only when operator opts in interactively), `superpowers:writing-plans` or `/plan` (planning), `superpowers:requesting-code-review` or `/skill:domain-review` (review). Consults `_internal/disposition` and `_internal/repo-delivery`.
