---
name: prd-validate
description: 'Turn an existing story, BRD, PRD, or AERS draft into an implementation-ready
  AERS. Interviews the author, closes structural and semantic ambiguity against the
  AERS and ontology rubrics, generates missing sections. Use before planning, /kickoff,
  or /execute-prd. When NOT to Use: vague intent without a problem statement yet (use
  /goal first to validate the outcome before writing requirements); no artifact at
  all (use /prd-create, which owns the blank start and writes the PRD folder).'
whenToUse: 'Turn an existing story, BRD, PRD, or AERS draft into an implementation-ready
  AERS. Interviews the author, closes structural and semantic ambiguity against the
  AERS and ontology rubrics, generates missing sections. Use before planning, /kickoff,
  or /execute-prd. When NOT to Use: vague intent without a problem statement yet (use
  /goal first to validate the outcome before writing requirements); no artifact at
...
# /skill:prd-validate — AERS Readiness Gate
...
artifact from an idea is not this skill's job — that belongs to `/skill:prd-create`.
Complements `/skill:prd-acceptance` (post-implementation validation).

**Use before planning, `/skill:kickoff`, or `/skill:execute-prd`** when the requirements are still ambiguous.
...
- Before planning, `/skill:kickoff`, or `/skill:execute-prd` on a new feature

## When NOT to Use

- Requirements are already implementation-ready — skip to `/skill:execute-prd` or `/skill:kickoff`
- You have only an idea and no artifact yet — use `/skill:prd-create`, which owns the blank-start interview and writes the PRD folder (`docs/prds/<slug>/`, including its `ONTOLOGY.md`)
- You want to build or extend an ontology interactively — use `/skill:prd-create` or `/skill:prd-create --extend`
- Verifying completed work — use `/skill:prd-acceptance`
...
- `--full-spec` — **deprecated.** Recognised for one release; prints a pointer to `/skill:prd-create` and does not run. See **Full spec mode** below.
...
> "No requirements artifact found. `/skill:prd-create` owns the blank start — it runs
> the interview and writes `docs/prds/<slug>/` including `ONTOLOGY.md`. Run that
> first, then `/skill:prd-validate` on what it produces."
...
- /skill:execute-prd <path>  (if ready)
- /skill:prd-create  (if the ontology is Absent on a non-trivial domain — there is no
  ONTOLOGY.md to extend, so this writes a new PRD folder)
- Step 3.5 closure pass here, then /skill:prd-create --extend <path> only if the
...
> `--full-spec` is deprecated. `/skill:prd-create` owns the blank-start interview and
> writes the PRD folder — `docs/prds/<slug>/`, including its `ONTOLOGY.md`. Run
> `/skill:prd-create` for a new spec, or `/skill:prd-validate <path>` (no flag) to harden an
> existing one.
...
- Do NOT drift into implementation planning — that belongs in `/skill:execute-prd`.

## Contract

- **Inputs:** `<path>` to a requirements artifact, OR the artifact pasted into the conversation; the sibling `ONTOLOGY.md` when one exists, plus the prior ontology named by its `extends:` in `feature` / `rewrite` mode. Optional flags: `--refine-problem`; `--full-spec` (deprecated — recognised, points at `/skill:prd-create`, does not run). Embeds both rubrics: `_internal/aers-readiness` for structure and `_internal/ontology-readiness` for semantics.
- **Preconditions:** human operator is at the keyboard — this is an interactive interview, not a gate. Callers (`/skill:kickoff`, `/skill:execute-prd`) MUST NOT auto-invoke this skill from a non-interactive context. A requirements artifact exists; a blank start belongs to `/skill:prd-create`.
...
- **Failure modes:** non-interactive context detected → refuse to start and suggest `_internal/aers-readiness` for a deterministic score instead; user says "you choose" on a non-default question → propose a default and ask for confirmation, do not silently pick; ontology absent on a non-trivial domain → report the gap and recommend a bare `/skill:prd-create`, never `--extend` (there is no `ONTOLOGY.md` to extend), and **do not fabricate** an ontology to close it; ontology `Partial` → settle what the Step 3.5 closure pass can settle here (`unknown` rows and incomplete mandatory core on entities the ontology already carries) and recommend `/skill:prd-create --extend` only when the remaining gap is new entities; `revision` entry in the Extension Log → surface it rather than scoring past it.
