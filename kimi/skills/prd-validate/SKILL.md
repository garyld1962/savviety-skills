---
name: prd-validate
description: 'Turn a rough story, BRD, AERS draft, or idea into an implementation-ready
  AERS. Interviews the author, closes ambiguity, generates missing sections. Use before
  planning, /kickoff, or /execute-prd. When NOT to Use: vague intent without a problem
  statement yet (use /goal first to validate the outcome before writing requirements).'
whenToUse: 'Turn a rough story, BRD, AERS draft, or idea into an implementation-ready
  AERS. Interviews the author, closes ambiguity, generates missing sections. Use before
  planning, /kickoff, or /execute-prd. When NOT to Use: vague intent without a problem
  statement yet (use /goal first to validate the outcome before writing requirements).'
arguments:
...
**Purpose:** Take a requirements artifact (story, BRD, PRD, partial AERS, or plain-language ask) and turn it into an implementation-ready AERS through focused interview. Complements `/skill:prd-acceptance` (post-implementation validation).
...
- Requirements are already implementation-ready — skip to `/plan` or `/skill:kickoff`
...
- **Preconditions:** human operator is at the keyboard — this is an interactive interview, not a gate. Callers (`/skill:kickoff`, `/skill:execute-prd`) MUST NOT auto-invoke this skill from a non-interactive context.
