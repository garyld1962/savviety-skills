---
name: prd-acceptance
description: Validate implementation against PRD acceptance criteria. Reads a PRD,
  extracts checkboxes, verifies each with evidence, produces a pass/fail scorecard.
whenToUse: Validate implementation against PRD acceptance criteria. Reads a PRD, extracts
  checkboxes, verifies each with evidence, produces a pass/fail scorecard.
---


# /skill:prd-acceptance — PRD Acceptance Validation
...
- PRD is still a draft — use `/skill:prd-validate` first
- You need a general code review — use `/skill:domain-review`
...
`no-acceptance-criteria` finding and suggest `/skill:prd-validate` — do not invoke
...
Next steps:
  ...
  3. Run `/skill:prd-acceptance --recheck` after fixes
```
...
- **Inputs:** path to the PRD/AERS to verify against (or the resolution order under `## Arguments`); optionally the sibling `ONTOLOGY.md`, whose `settled` rows become `OC-` items per Step 2.5 and whose table shapes and item states are defined by `_internal/ontology-readiness`; the built / running application. Calls `/skill:domain-review` when defects warrant a structured list; it does not call `/skill:prd-validate` — an artifact with no acceptance criteria halts with `no-acceptance-criteria` and suggests `/skill:prd-validate` instead.
