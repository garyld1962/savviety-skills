---
name: execute-plan
description: Use when the user has a written implementation plan file (typically produced
  by /execute-prd, in the plan-format contract) and wants it executed — phrases like
  'execute the plan', 'run docs/plans/X.md', 'resume the plan'. Not for writing plans
  (use /execute-prd) or trivial single-file edits.
whenToUse: Use when the user has a written implementation plan file (typically produced
  by /execute-prd, in the plan-format contract) and wants it executed — phrases like
  'execute the plan', 'run docs/plans/X.md', 'resume the plan'. Not for writing plans
  (use /execute-prd) or trivial single-file edits.
type: flow
...
# /skill:execute-plan — Plan Executor (judgment layer)
...
2. **Validate plan — refuse contract.** Run `/skill:validate-plan <path>`.
...
4. **Pre-execution clarification (codebase-aware).**
   ...
   **This is NOT a re-validation of the plan.** `/skill:validate-plan`
...
change. `confidence: high|medium|low` lets `/skill:process-tune` weight
...
`/skill:process-tune` (the symmetric consumer skill) reads this index,
...
- No plan exists — use `/skill:execute-prd` (PRD input) or
