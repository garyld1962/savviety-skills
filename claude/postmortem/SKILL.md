...
**Purpose:** produce the same postmortem `/execute-plan`
...
- A past `/execute-plan` run completed without firing the postmortem
...
- The run hasn't completed yet — `/execute-plan`'s postmortem step
...
- Do not invent a different schema or taxonomy. The taxonomy is
  ...
  `claude/execute-plan/SKILL.md` so that `/process-tune` can
