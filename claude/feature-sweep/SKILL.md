...
# /feature-sweep — Integrate New Platform Features into Installed Skills
...
- Invokes `/execute-prd`, `/execute-plan`, or similar at the end
...
**For `/batch`:** find the Phase/Step that currently invokes `/execute-prd` or `/execute-plan` — that's where the `/batch` alternative goes.
