---
description: >-
  Investigate a bug from reproduction through root cause analysis. Produces a
  structured triage report with classification, risk assessment, and recommended
  next step. DOES NOT write fixes — use kickoff or execute-plan for that. Use
  when you need systematic investigation, not when the fix is obvious.
...
**Purpose:** Systematically investigate a bug from reproduction through root cause identification. Produces a structured triage report with classification, risk assessment, and a recommended path forward. This prompt investigates — it does NOT write fixes. Adapts to any codebase by reading `copilot-instructions.md`.
...
- You want a structured handoff to hotfix or execute-plan

## When NOT to Use

- You already know the root cause and just need to fix it — use hotfix or kickoff
...
- `--linear <issue-id>` — fetch details from Linear via the Linear MCP tool
- `--quick` — skip regression analysis (Step 5), produce a shorter report

## Step 1: Project Discovery

Read `copilot-instructions.md` from the repo root to learn:
...
If `--ado <item-id>` or `--linear <issue-id>` was provided, use the work-item prompt to retrieve the full details. Extract:
...
Determine when this bug was introduced by running git log on the affected files:

```bash
git log --oneline -20 -- <affected-files>
```

If a specific commit is suspect, inspect it:
...
| **execute-plan** | Fix complexity is Complex OR regression risk is High |
...
1. **Investigate first, fix later.** This prompt does NOT write fixes. It produces a diagnosis.
