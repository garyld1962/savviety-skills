---
description: >-
  Interrogates to discover the single-sentence product or architectural thesis,
  then audits all planned features and decisions against it. Two modes: product
  (what is this application for, who is it for, what does it claim?) and arch
  (what is the one technical commitment shaping this system?). Use before
  planning or writing handoff docs — not after implementation. Adapted from
  Corey Mandel's screenwriting discipline: applications fail the same way
  unfocused movies fail, describing a topic instead of a claim.
argument-hint: '[product|arch] [optional context or doc path]'
agent: agent
---

# Thesis — Product & Architectural Thesis
...
- Before writing a handoff doc — the thesis goes at the top
...
- _(none)_ — ask the user which mode. Do not guess.

## Output

Thesis + audit only. This prompt deliberately does **not** produce a PRD, design doc, or handoff doc. Those are separate artifacts; the thesis is the referee they all answer to. The output is meant to be pasted into the top of a handoff doc, PRD, or design doc — not replace them.
...
- **Phase discipline.** Do not audit before the thesis is locked. Do not lock the thesis before it passes the tests.
...
- **Respect the composition.** This prompt produces a thesis artifact, not a handoff doc. Offer at the end: *"Want to hand this to /research now? The handoff doc goes on top of this."*
- **Stop when done.** Thesis produced, audit complete, user confirms. Don't keep grilling.
- If optional context or a doc path was passed as an argument, read that file before asking questions — it may answer Phase 1 questions before you ask them.
...
- **The missing product thesis.** Symptom: a crisp architectural thesis but a fuzzy user. Fix: run in `product` mode. These are different artifacts.
