---
name: professional-review-rubric
description: >
  Seniority-calibrated craft grading rubric used by code-review-professional.
  Copilot auto-loads this skill whenever a professional-grade code review is
  requested or when prompts/review/code-review-professional.prompt.md is
  active. Provides the authoritative 7-axis grade definitions (junior through
  staff) and citation requirements. Do not use this for defect triage or
  general code feedback — it grades craft, not correctness.
---
...
1. Read the diff in full before scoring any axis. First-pass reading is for orientation; scoring comes after.
2. For each axis, find 2–3 specific line citations that support the grade. If you cannot find citations, skip that axis.
3. Score each axis independently. Resist averaging — a single weak axis at `junior` on a otherwise `senior` PR is meaningful signal.
4. Apply the overcomplication cap on Judgment before finalising: re-read the PR scope, ask whether the solution size matches the problem size.
5. Check for split decisions: if the PR touches multiple components (e.g. API layer + UI), score each separately.
6. Derive the overall component grade: the lowest axis score sets the ceiling, except that `staff` requires evidence on all 7 axes at `senior` or above.
7. Separate any bugs or defects found during review into the `domain-review` output — do not let defect severity influence axis grades.
